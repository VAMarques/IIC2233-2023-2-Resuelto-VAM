...
class Jugador(Thread):
    ...
    def __init__(
        ...
        # Completar
        self.daemon = False  # Debe ser False si el programa no puede terminar antes que esta clase
    ...
    def intentar_capturar_bandera(self) -> None:
        # Completar
        logrado = self.lock_bandera.acquire(blocking=False)
        if logrado and (self.bandera.nombre_dueño is None):
            self.capturar_bandera()
            self.lock_bandera.release()
        elif logrado and (self.bandera.nombre_dueño is not None):
            self.lock_bandera.release()
        if self.tiene_bandera:
    ...
    def intentar_robar_bandera(self) -> bool:
        ...
        if random() < self.PROBABILIDAD_ROBAR:  # Verdadero si se roba la bandera
